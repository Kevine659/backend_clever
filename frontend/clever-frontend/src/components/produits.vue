...
const getFavoriteCount = (product) => {
    ...
    if (
        favoriteCounts.value[productId] !== undefined
    ) {
        return Number(
            favoriteCounts.value[productId]
        ) || 0
    }
...
const loadGuestFavorites = () => {
    try {
        ...
        console.error(
            "Erreur chargement favoris invité :",
            err
        )
...
// on recupere l id du produit du favorite
const extractFavoriteProductId = (item) => {
    ...
    if (item.product_detail) {
        if (
            typeof item.product_detail === "object" &&
    ...
    if (item.product_data) {
        if (
            typeof item.product_data === "object" &&
...
const loadFavorites = async () => {
    try {
        ...
        // compteur du nombre de fois que  les produits on ete ajouter au favorite
...
const addFavorite = async (product) => {
    ...
    if (favoriteProcessing.value !== null) {
        return
    }
            ...
            <div v-else-if="products.length > 0" class="products-grid">
                <article v-for="product in products" :key="product.id" class="product-card">
                    <div class="product-image-container">
                        ...
                        <button type="button" class="favorite-button" :class="{ active: isFavorite(product.id) }" :title="isFavorite(product.id)
                                ? 'Retirer des favoris'
                                : 'Ajouter aux favoris'
                            " :disabled="favoritesLoading" @click.stop="toggleFavorite(product)">
                            <i :class="isFavorite(product.id)
                                    ? 'fa-solid fa-heart'
                                    : 'fa-regular fa-heart'
                                "></i>
                        </button>
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-bottom">
                            ...
                            <button type="button" class="add-to-cart-button" :disabled="Number(product.stock) <= 0"
                                title="Ajouter au panier" @click="addToCart(product)">
                                <i class="fa-solid fa-cart-shopping"></i>
                            ...
                            <div class="cart-products">
                                <div v-for="item in cart" :key="item.id" class="cart-item">
                                    <div class="cart-item-image">
                                        <img v-if="item.image" :src="getProductImage(item.image)" :alt="item.name"
                                    ...
                                    <div class="cart-item-info">
                                        ...
                                        <div class="cart-item-bottom">
                                            <div class="cart-quantity">
                                                ...
                                                <button type="button" @click="increaseCartQuantity(item)" :disabled="item.type === 'product' &&
                                                    item.stock > 0 &&
...
.header-cart-button:hover .cart-count,
.header-favorite-button:hover .favorite-header-count {
    background: #fff;
    color: #38B04C;
}
...
.product-card:hover .add-to-cart-button span {
    display: inline;
}
