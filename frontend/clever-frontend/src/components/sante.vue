...
const isHealthProduct = (product) => {
    ...
    return categoryName === "santé & bien-être" ||
        categoryName === "sante & bien-etre" ||
        categoryName === "santé et bien-être" ||
        categoryName === "sante et bien-etre"
}
...
<h2>Nos produits de <span>santé</span></h2>
</div>
<div class="header-actions">
<RouterLink to="/favoris" class="header-favorite-button">
<i class="fa-regular fa-heart"></i>
<span>Favoris</span>
<strong v-if="favoritesCount > 0" class="favorite-header-count">{{ favoritesCount }}</strong>
</RouterLink>
<button type="button" class="header-cart-button" @click="showCart = true">
<i class="fa-solid fa-cart-shopping"></i>
<span>Panier</span>
<strong v-if="cartItemsCount > 0" class="cart-count">{{ cartItemsCount }}</strong>
</button>
...
<p>Impossible de charger les produits santé.</p>
<button type="button" @click="loadProducts">Réessayer</button>
</div>
...
<span v-if="Number(product.stock) <= 0" class="out-of-stock">Rupture de stock</span>
</div>
<div class="product-info">
<span v-if="product.subcategory_name" class="product-category">{{ product.subcategory_name }}</span>
...
<strong class="current-price sale-price">{{ formatPrice(product.current_price ?? product.promotional_price ?? product.price) }}</strong>
<span class="old-price">{{ formatPrice(product.price) }}</span>
</template>
<strong v-else class="current-price">{{ formatPrice(product.price) }}</strong>
</div>
...
<i class="fa-solid fa-heart-pulse"></i>
<p>Aucun produit de la catégorie Santé & Bien-être disponible.</p>
</div>
</div>
</section>
...
<div>
<h3>Mon panier</h3>
<span>{{ cartItemsCount }} {{ cartItemsCount > 1 ? "articles" : "article" }}</span>
</div>
</div>
...
<div v-else class="cart-no-image"><i class="fa-solid fa-image"></i></div>
</div>
<div class="cart-item-info">
<span class="cart-item-type">{{ item.type === "range" ? "GAMME" : "PRODUIT" }}</span>
<h4>{{ item.name }}</h4>
...
<strong class="cart-item-total">{{ formatPrice(getItemTotal(item)) }}</strong>
</div>
</div>
...
<button type="button" class="continue-button" @click="closeCart">Continuer mes achats</button>
...
.product-image-container,.skeleton-image{height:205px}
}
...
.add-to-cart-button{height:27px;min-width:27px;padding:0 7px}
.product-card:hover .add-to-cart-button{min-width:98px}
.view-details-button{bottom:10px;padding:6px 10px;font-size:8px}
.favorite-counter{top:42px;right:10px}
