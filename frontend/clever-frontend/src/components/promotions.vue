...
// Extraction de l'id du produit favori
...
// Chargement uniquement des produits en promotion
const loadProducts = async () => {
    try {
        ...
        products.value = data.filter(product => {
            ...
            return {
                ...product,
                favorite_count: count !== undefined ? Number(count) || 0 : Number(product.favorite_count || product.favorites_count || 0) || 0
            }
        })
        console.log("Produits en promotion :",products.value)
    } catch (err) {
...
Produits actuellement disponibles à prix réduit.
</p>
</div>
...
<RouterLink to="/boutique" class="view-all-button">
...
<p>Il n'y a actuellement aucun produit en promotion.</p>
<RouterLink to="/boutique" class="promotion-shop-button">
...
.promotion-description{margin:8px 0 0;color:#999;font-size:11px}
...
.favorite-button:hover,.favorite-button.active{background:#38B04C;color:#fff}
...
.current-price{color:#38B04C;font-size:12px;font-weight:700}
...
.products-message h3{margin:0;color:#222;font-family:Georgia,"Times New Roman",serif;font-size:20px}
.products-message p{margin:0;font-size:12px}
.products-message button,.promotion-shop-button{padding:8px 15px;border:0;border-radius:4px;background:#38B04C;color:#fff;font-size:10px;cursor:pointer;text-decoration:none}
.promotion-shop-button{display:inline-flex;align-items:center;gap:7px}
...
.cart-item{position:relative;display:flex;gap:12px;padding:12px;border:1px solid #eee;border-radius:8px;background:#fff}
...
.cart-quantity button{width:27px;height:100%;border:0;background:#fff;color:#555;cursor:pointer}
...
.empty-cart h3{margin:0 0 8px;color:#222;font-family:Georgia,"Times New Roman",serif;font-size:21px}
...
@media(max-width:991px){.products-header{align-items:flex-start;flex-direction:column;gap:14px}.header-actions{width:100%}.products-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media(max-width:768px){.products-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}.cart-panel{width:430px}}
@media(max-width:576px){.products-section{padding:35px 0 45px}.products-header h2{font-size:27px}.header-actions{align-items:stretch;flex-direction:column}.header-cart-button,.header-favorite-button,.view-all-button{width:100%}.products-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.product-image-container,.skeleton-image{height:175px}.product-image{padding:7px}.product-info{padding:9px}.product-name a{font-size:12px}.current-price{font-size:10px}.cart-panel{width:100%;max-width:100%}.cart-header{padding:15px}.cart-content{padding:12px}.cart-footer{padding:15px}.cart-item-image{width:70px;height:70px}}
</style>