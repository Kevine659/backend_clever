...
const loadProducts = async () => {
    try {
        loading.value = true
        error.value = false
        const response = await api.get("/products/?category=3")
        console.log("Réponse produits Fertilité & Bien-être :",response.data)
        const data = response.data?.results || response.data || []
        if (!Array.isArray(data)) {
            products.value = []
            return
        ...
        products.value = data.filter(product => {
            ...
            if (product.category_id !== undefined && Number(product.category_id) !== 3) return false
            if (product.category !== undefined && typeof product.category !== "object" && Number(product.category) !== 3) return false
            return true
        }).map(product => {
        ...
        console.log("Produits Fertilité & Bien-être :",products.value)
    } catch (err) {
        console.error("Erreur chargement produits Fertilité & Bien-être :",err)
        error.value = true
        products.value = []
    } finally {
        loading.value = false
...
<p>Impossible de charger les produits de Fertilité & Bien-être.</p>
...
<button type="button" class="favorite-button" :class="{active:isFavorite(product.id)}" :title="isFavorite(product.id)?'Retirer des favoris':'Ajouter aux favoris'" :disabled="favoritesLoading || favoriteProcessing === product.id" @click.stop="toggleFavorite(product)">
...
<i class="fa-solid fa-heart-pulse"></i>
<p>Aucun produit de la catégorie Fertilité & Bien-être disponible pour le moment.</p>
</div>
</div>
</section>
...
.skeleton-image,.skeleton-line{background:linear-gradient(90deg,#f5f5f5 25%,#fff 50%,#f5f5f5 75%);background-size:200% 100%;animation:skeletonAnimation 1.5s infinite}
.skeleton-image{width:100%;height:220px}
.skeleton-content{padding:13px}
.skeleton-line{height:9px;margin-bottom:8px;border-radius:3px}
...
.products-header{align-items:flex-start;flex-direction:column;gap:14px}
.header-actions{width:100%}
.products-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
}
@media(max-width:768px){
.products-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}
.product-image-container,.skeleton-image{height:215px}
}
@media(max-width:576px){
.products-section{padding:35px 0 45px}
.products-header h2{font-size:27px}
.header-actions{align-items:stretch;flex-direction:column}
.header-cart-button,.header-favorite-button,.view-all-button{width:100%}
.products-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}
.product-image-container,.skeleton-image{height:175px}
