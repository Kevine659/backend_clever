...
const getFavoriteProductId=item=>{
    ...
    if(item.product_detail?.id){
        const id=Number(item.product_detail.id)
        if(Number.isFinite(id)&&id>0)return id
    }
    if(item.product_data?.id){
...
const loadFavoriteCounts=async()=>{
    try{
        ...
        console.error(
            "Erreur chargement compteurs favoris :",
            err
        )
    }
}
...
const loadFavorites=async()=>{
    try{
        ...
        if(!Array.isArray(favoritesData)||!Array.isArray(productsData)){
            favorites.value=[]
            return
        }
        ...
        favorites.value=uniqueFavoriteIds
            .map(productId=>{
                ...
                return{
                    ...
                    favorite_count:
                        favoriteCounts.value[String(product.id)]!==undefined
                            ?Number(
                                favoriteCounts.value[
                                    String(product.id)
                                ]
        ...
        console.log(
            "Favoris complets avec produits :",
            favorites.value
        )
...
const removeFavorite=async product=>{
    ...
    try{
        ...
        if(!isAuthenticated.value){
            const savedFavorites=favorites.value.map(
                item=>Number(item.id)
            )
...
<span class="favorites-small-title">
...
v-for="n in 4"
:key="n"
class="favorite-card skeleton-card"
>
...
:disabled="favoriteProcessing===product.id"
...
<style scoped>
.favorites-section{width:100%;min-height:70vh;padding:45px 0 70px;background:#fff}
.favorites-header{width:100%;display:flex;align-items:flex-end;justify-content:space-between;gap:25px;margin-bottom:30px}
.favorites-title-block{display:flex;flex-direction:column;align-items:flex-start}
.favorites-small-title{display:flex;align-items:center;gap:7px;margin-bottom:7px;color:#38B04C;font-size:10px;font-weight:700;letter-spacing:1.6px;text-transform:uppercase}
.favorites-small-title i{font-size:10px}
.favorites-header h1{margin:0;color:#202020;font-family:Georgia,"Times New Roman",serif;font-size:36px;font-weight:500;line-height:1.1}
.favorites-header h1 span{color:#38B04C}
.favorites-title-block p{margin:9px 0 0;color:#888;font-size:11px}
.favorites-header-actions{display:flex;align-items:center;gap:9px}
.back-shop-button,.cart-button{height:38px;display:inline-flex;align-items:center;justify-content:center;gap:7px;padding:0 14px;border:1px solid #dfe8e1;border-radius:20px;background:#fff;color:#333;font-size:10px;font-weight:600;text-decoration:none;cursor:pointer;transition:.25s ease}
.back-shop-button:hover,.cart-button:hover{background:#38B04C;border-color:#38B04C;color:#fff}
.cart-button{position:relative}
.cart-button strong{min-width:18px;height:18px;display:flex;align-items:center;justify-content:center;border-radius:50%;background:#38B04C;color:#fff;font-size:8px}
.cart-button:hover strong{background:#fff;color:#38B04C}
.favorites-grid{width:100%;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
.favorite-card{width:100%;overflow:hidden;border:1px solid #eee;border-radius:8px;background:#fff;transition:.3s ease}
.favorite-card:hover{transform:translateY(-4px);border-color:#e4e9e4;box-shadow:0 12px 30px rgba(0,0,0,.07)}
.favorite-image-container{width:100%;height:220px;position:relative;overflow:hidden;background:#fff}
.favorite-image-link{width:100%;height:100%;display:flex;align-items:center;justify-content:center;text-decoration:none}
.favorite-image{display:block;width:100%;height:100%;padding:12px;object-fit:contain;transition:transform .45s ease}
.favorite-card:hover .favorite-image{transform:scale(1.05)}
.no-image{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;color:#aaa;font-size:11px}
.no-image i{font-size:25px}
...
.favorite-card:hover .view-details-button{opacity:1;visibility:visible;transform:translate(-50%,0)}
...
.remove-favorite-button{position:absolute;top:9px;right:9px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;border:0;border-radius:50%;background:#38B04C;color:#fff;box-shadow:0 3px 10px rgba(0,0,0,.08);cursor:pointer;z-index:11;transition:.25s ease}
.remove-favorite-button:hover{background:#2f963f;transform:scale(1.08)}
.remove-favorite-button:disabled{opacity:.5;cursor:not-allowed}
.remove-favorite-button i{font-size:11px}
.favorite-count{position:absolute;top:44px;right:12px;min-width:30px;height:18px;display:flex;align-items:center;justify-content:center;gap:4px;padding:0 6px;border-radius:10px;background:#fff;color:#555;box-shadow:0 3px 10px rgba(0,0,0,.08);font-size:8px;font-weight:600;z-index:10}
.favorite-count i{color:#38B04C;font-size:8px}
.favorite-count span{font-size:8px}
...
.product-rating{display:flex;gap:2px;margin-bottom:9px}
...
.favorite-card:hover .add-to-cart-button{min-width:88px;border-color:#38B04C;background:#38B04C;color:#fff}
.favorite-card:hover .add-to-cart-button span{display:inline}
...
.empty-favorites{width:100%;min-height:400px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.empty-favorites-icon{width:85px;height:85px;display:flex;align-items:center;justify-content:center;margin-bottom:18px;border-radius:50%;background:#f1f8f3;color:#38B04C}
.empty-favorites-icon i{font-size:31px}
.empty-favorites h2{margin:0 0 8px;color:#222;font-family:Georgia,"Times New Roman",serif;font-size:24px;font-weight:500}
.empty-favorites p{margin:0;color:#999;font-size:11px}
.discover-button{display:inline-flex;align-items:center;gap:8px;margin-top:20px;padding:11px 17px;border-radius:5px;background:#38B04C;color:#fff;font-size:11px;font-weight:600;text-decoration:none}
.discover-button:hover{background:#2f963f;color:#fff}
.favorites-message{width:100%;min-height:300px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.message-icon{width:65px;height:65px;display:flex;align-items:center;justify-content:center;margin-bottom:15px;border-radius:50%;background:#f1f8f3;color:#38B04C}
.message-icon i{font-size:24px}
.favorites-message h2{margin:0 0 7px;color:#222;font-family:Georgia,"Times New Roman",serif;font-size:21px;font-weight:500}
.favorites-message p{margin:0;color:#999;font-size:11px}
.favorites-message button{display:inline-flex;align-items:center;gap:7px;margin-top:17px;padding:9px 15px;border:0;border-radius:5px;background:#38B04C;color:#fff;font-size:10px;cursor:pointer}
...
.skeleton-line{height:9px;margin-bottom:9px;border-radius:3px;background:linear-gradient(90deg,#f5f5f5 25%,#fff 50%,#f5f5f5 75%);background-size:200% 100%;animation:skeletonAnimation 1.5s infinite}
...
.cart-close-button{width:34px;height:34px;display:flex;align-items:center;justify-content:center;border:0;border-radius:50%;background:#f6f6f6;color:#555;cursor:pointer}
...
.cart-item{position:relative;display:flex;gap:12px;padding:12px;border:1px solid #eee;border-radius:8px}
...
.cart-item-image img{width:100%;height:100%;padding:6px;object-fit:contain}
...
.cart-item-info h4{margin:2px 0 7px;color:#222;font-family:Georgia,"Times New Roman",serif;font-size:14px}
...
.remove-cart-item{position:absolute;top:9px;right:9px;border:0;background:transparent;color:#aaa;cursor:pointer}
.remove-cart-item:hover{color:#d9534f}
.cart-footer{padding:18px 20px;border-top:1px solid #eee;background:#fff}
...
.summary-total{margin-top:12px;padding-top:13px;border-top:1px solid #eee;color:#222;font-size:14px}
.summary-total strong{color:#38B04C;font-size:17px}
.checkout-button{width:100%;height:43px;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:16px;border-radius:5px;background:#38B04C;color:#fff;font-size:12px;font-weight:600;text-decoration:none}
...
.favorites-header{align-items:flex-start;flex-direction:column}
.favorites-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
}
@media(max-width:768px){
.favorites-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}
}
@media(max-width:576px){
.favorites-section{padding:35px 0 45px}
.favorites-header h1{font-size:29px}
.favorites-title-block p{line-height:1.6}
.favorites-header-actions{width:100%;flex-direction:column;align-items:stretch}
.back-shop-button,.cart-button{width:100%}
.favorites-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}
.favorite-image-container,.skeleton-image{height:175px}
.favorite-image{padding:7px}
.favorite-info{padding:9px}
